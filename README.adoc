...
## DIALLO Thièrno Amirou && BARRY Thierno Boubacar
WARNING: NE PAS OUBLIER DE MENTIONNER LES DEUX NOMS SI VOUS ETES EN BINOME!

#DIALLO Thièrno Amirou# et #BARRY Thierno Boubacar#
...
Non, il n'est pas structuré en couche. Pas facile à maintenir car tous les domaines sont melangés (client, serveur, base de données). De plus 
il ya une confusion dans la logique metier. 

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se trouve dans la classe TodoListController.java
*Cette règle est-elle facilement testable par un test unitaire ?*
Non, elle n'est pas facile à tester. A l'interieur de la methode on y trouve plusieurs autres fonctionnalités.
* Lancer une base PostgreSQL en Docker dans un terminal (on lance ici la base en mode interactif pour visualiser son activité. Pour la lancer en tâche de fond, remplacer les options `it` par `d` comme 'daemon'):
```bash
docker run -it -e POSTGRES_PASSWORD=password -p 5432:5432 postgres
```
*Expliquer cette ligne de commande (y compris les options utilisées)*
Docker run permet de lancer un conteneur postgres
-it lancer le conteneur en mode interactif pour visualiser son activité
-e permet de modifier la variable d'environnment POSTGRES_PASSWORD à password 
-p permet de specifier le port
* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
La meilleure façon de l'injecter est de l'injecter dans le constructeur
* Modifier le code en conséquence.
todoItemRepository était null car il n'était pas injecté
...
_L'architecture hexagonale est un patron d'architecture utilisé dans le domaine de la conception des logiciels. Elle respecte les principes SOLID. Elle s'appuie sur 3 grands principes :
...
Il s'agit d'ecrire de tests d'integration.
*Que teste-on dans ce cas ?*
L'interaction entre les differents adapteurs.
